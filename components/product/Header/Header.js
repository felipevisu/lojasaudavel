...
export function Header({category, total}){
  ...
  return(
    <div className="relative flex w-full items-center border my-6">
      <div>
        <button onClick={() => setFilterOpen(true)} className="focus:outline-none px-4 py-2 flex items-center border-r hover:bg-gray-200">
      ...
      <div ref={sorter} className="ml-auto mr-0">
        ...
        <button onClick={() => setDropdown(!dropdown)} className="focus:outline-none px-4 py-2 flex items-center border-l hover:bg-gray-200">
        ...
        <div className={` ${ dropdown ? "block" : "hidden"}`}>
          <div className="absolute z-40 w-72 right-0 bg-white border p-4" style={{marginRight: -1}}>
            {SORT_FIELDS.map((field, key) =>
              <button key={key} onClick={handleChange} value={field.value} className="focus:outline-none w-full text-left py-1">{field.name}</button>
            )}
          </div>
        </div>
