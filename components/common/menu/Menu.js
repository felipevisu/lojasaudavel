...
export function Menu(){
  ...
  useEffect(() => {
    if(site.menuOpen){
      site.setMenuOpen(false)
    }
  }, [router])

  return(
    <>
      <div onClick={() => site.setMenuOpen(false)} className={` ${site.menuOpen ? 'block' : 'hidden'} fixed top-0 left-0 w-full h-full bg-black z-40 opacity-50 `} />
        ...
        <div className="px-3 absolute bottom-0 top-12 pb-3 overflow-auto w-full">
          <div className="grid grid-cols-2 rounded border mx-3 mb-4 text-center">
            {auth.user === null 
              ...
              :
              <button className="appearance-none focus:outline-none" onClick={() => logout()}>
                <div className="p-2 border-r justify-center flex items-center cursor-pointer transition transition-all hover:text-green-500">
